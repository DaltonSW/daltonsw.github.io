...
func findOpenEntries(archiveDir, gamesDir string, games []GameSummary, thresholdDays int, includePlaying bool) ([]OpenEntry, error) {
	...
	for _, g := range games {
		...
		for _, p := range pf.Views() {
			...
			last, err := time.ParseInLocation("2006-01-02", closeOn, siteLocation)
			if err != nil {
				continue
			}
...
func archiveLastPlayed(archiveDir string, g GameSummary) string {
	best := ""
	for _, link := range g.ProviderLinks() {
		if link.ID == "" {
			continue
		}
		rec, err := LoadRecord(archiveDir, link.Provider, link.ID)
		if err != nil || rec == nil {
			continue
		}
