...
func daysAgo(n int) string {
	return time.Now().In(model.SiteLocation).AddDate(0, 0, -n).Format("2006-01-02")
}
...
func nUnlocked(prefix string, n int) []model.ArchivedAchievement {
	out := make([]model.ArchivedAchievement, n)
	for i := range out {
		out[i] = model.ArchivedAchievement{
			Key: fmt.Sprintf("%s-%d", prefix, i), Name: fmt.Sprintf("%s-%d", prefix, i),
			Unlocked: true, Date: "2024-01-01T00:00:00-06:00",
		}
	}
	return out
}
...
// nothing to suggest, including endless games, which are supposed to
// stay "playing" indefinitely by design.
func TestFindStaleCandidates_FiltersToEligibleGames(t *testing.T) {
	archiveDir := t.TempDir()
	if _, err := model.SaveRecord(archiveDir, model.ProviderSteam, "1", "Quiet Game", &model.ProviderRecord{
	...
	games := []model.GameSummary{
		...
		{Slug: "session-game", Title: "Session Game", Status: "endless", SteamAppID: "1"},
...
func TestFindStaleCandidates_RecentlyPlayedIsExcluded(t *testing.T) {
	archiveDir := t.TempDir()
	if _, err := model.SaveRecord(archiveDir, model.ProviderSteam, "1", "Recent Game", &model.ProviderRecord{
		Total: 10, Achievements: nUnlocked("a", 3), LastPlayed: daysAgo(5),
	}); err != nil {
		t.Fatal(err)
	}
	games := []model.GameSummary{{Slug: "recent", Status: "playing", SteamAppID: "1"}}
...
func TestFindStaleCandidates_SuggestsByCompletion(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			archiveDir := t.TempDir()
			if _, err := model.SaveRecord(archiveDir, model.ProviderSteam, "1", "Game", &model.ProviderRecord{
				Total: tc.total, Achievements: nUnlocked("a", tc.unlocked), LastPlayed: daysAgo(45),
			}); err != nil {
				t.Fatal(err)
			}
			games := []model.GameSummary{{Slug: "g", Status: "playing", SteamAppID: "1"}}
...
func TestFormatStaleCard_ShowsSuggestionAndConfidence(t *testing.T) {
	c := StaleCandidate{
		Game:       model.GameSummary{Title: "Hades", SteamAppID: "1145360"},
...
func TestFormatStaleCard_TagsDraftGames(t *testing.T) {
	c := StaleCandidate{
		Game:            model.GameSummary{Title: "Hades", SteamAppID: "1145360", Draft: true},
		SuggestedStatus: "dropped", Confidence: "medium",
	}
